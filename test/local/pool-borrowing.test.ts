...
describe('Pool with variable utilisation interest rates', () => {
    ...
    describe('Borrowing access', () => {
        ...
        it("should not allow non registered account to borrow", async () => {
            await expect(sut.connect(nonRegistered).borrow(toWei("1.0"))).to.be.revertedWith('Only the accounts authorised by borrowers registry may borrow');
        });
        ...
        it("should allow registered account to borrow", async () => {
            ...
            await expect(sut.connect(nonRegistered).borrow(toWei("1.0"))).to.be.revertedWith('Only the accounts authorised by borrowers registry may borrow');
            await expect(sut.connect(registered).borrow(toWei("1.0"))).to.be.revertedWith('Only the accounts authorised by borrowers registry may borrow');
            await expect(sut.connect(borrower).borrow(toWei("1.0"))).to.be.revertedWith('Only the accounts authorised by borrowers registry may borrow');
