...
const {
  ...
  deployFacet,
  replaceFacet,
} = require("../../../tools/diamond/deploy-diamond");
...
const wavaxAbi = ["function deposit() public payable", ...erc20ABI];

const LIQUIDATOR_PRIVATE_KEY = fs
  .readFileSync(path.resolve(__dirname, "../../../tools/liquidation/.private"))
  .toString()
  .trim();
const rpcProvider = new ethers.providers.JsonRpcProvider();
const liquidatorWallet = new ethers.Wallet(LIQUIDATOR_PRIVATE_KEY).connect(
  rpcProvider
);
...
describe("Test liquidator", () => {
  ...
  describe("A loan with debt and repayment", () => {
    ...
    before("deploy factory, exchange, wavaxPool and usdPool", async () => {
      ...
      exchange = await deployAndInitPangolinExchangeContract(
        owner,
        pangolinRouterAddress,
        supportedAssets
      );

      const variableUtilisationRatesCalculatorERC20 = (await deployContract(
        owner,
        VariableUtilisationRatesCalculatorArtifact
      )) as VariableUtilisationRatesCalculator;
      ...
      wavaxTokenContract = new ethers.Contract(
        wavaxTokenAddress,
        wavaxAbi,
        provider
      );
      usdTokenContract = new ethers.Contract(
        usdTokenAddress,
        erc20ABI,
        provider
      );

      const borrowersRegistryERC20 = await new OpenBorrowersRegistry__factory(
        owner
      ).deploy();
      const depositIndexERC20 = (await deployContract(
        owner,
        CompoundingIndexArtifact,
        [wavaxPool.address]
      )) as CompoundingIndex;
      const borrowingIndexERC20 = (await deployContract(
        owner,
        CompoundingIndexArtifact,
        [wavaxPool.address]
      )) as CompoundingIndex;
      ...
      await wavaxTokenContract
        .connect(depositor)
        .deposit({ value: toWei("1000") });
      await wavaxTokenContract
        .connect(depositor)
        .approve(wavaxPool.address, toWei("1000"));
      ...
      await deployFacet(
        "MockSmartLoanLogicFacetAlwaysSolvent",
        diamondAddress,
        [],
        ltvLib.address
      );
      await deployFacet(
        "SmartLoanLiquidationFacet",
        diamondAddress,
        ["liquidateLoan", "unsafeLiquidateLoan", "closeLoan"],
        ltvLib.address
      );
    ...
    it("should deploy a smart loan", async () => {
      ...
      loanFactory = await ethers.getContractFactory(
        "MockSmartLoanLogicFacetAlwaysSolvent",
        {
          libraries: {
      ...
      loan = (await loanFactory
        .attach(loanAddress)
        .connect(borrower)) as SmartLoanLogicFacet;

      wrappedLoan = WrapperBuilder.wrapLite(loan).usingPriceFeed(
    ...
    it("replace facet", async () => {
      const loanFactoryLiquidation = await ethers.getContractFactory(
        "SmartLoanLiquidationFacet",
        {
          libraries: {
            LTVLib: ltvLib.address,
      ...
      loan = (await loanFactoryLiquidation
        .attach(loanAddress)
        .connect(owner)) as SmartLoanLiquidationFacet;
