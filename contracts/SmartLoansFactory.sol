...
contract SmartLoansFactory is OwnableUpgradeable, IBorrowersRegistry {
  ...
  function createAndFundLoan(bytes32 _fundedAsset, address _assetAddress, uint256 _amount, bytes32 _debtAsset, uint256 _initialDebt) public virtual oneLoanPerOwner returns (SmartLoanDiamond) {
    ...
    ProxyConnector.proxyCalldata(address(smartLoan), abi.encodeWithSelector(SmartLoanLogicFacet.fund.selector, _fundedAsset, _amount), false);

    ProxyConnector.proxyCalldata(address(smartLoan), abi.encodeWithSelector(SmartLoanLogicFacet.borrow.selector, _debtAsset, _initialDebt), false);
