...
contract LiquidationFlashloan is FlashLoanReceiverBase, OwnableUpgradeable {
  ...
  function executeOperation(
    ...
    console.log(
      "before liquidation :",
      IERC20Metadata(_assets[0]).balanceOf(address(this))
    );
    ...
    // liquidate loan
    {
      (bool success, ) = address(liquidationFacet).call(
        abi.encodePacked(
      ...
      console.log(
        "after liquidation  :",
        IERC20Metadata(_assets[0]).balanceOf(_initiator)
      );
      console.log(
        "after liquidation  :",
        IERC20Metadata(_assets[0]).balanceOf(msg.sender)
      );
      console.log(
        "after liquidation  :",
        IERC20Metadata(_assets[0]).balanceOf(liquidator)
      );
      console.log(success);
    }
    ...
    for (uint32 i = 0; i < supportedTokens.length; i++) {
      ...
      if (index != -1) {
        ...
        console.log(
          "amounts + premiums :",
          _amounts[uint256(index)] + _premiums[uint256(index)]
        );
  ...
  function executeFlashloan(FlashLoanArgs calldata _args) public {
    ...
    console.log(
      "before flashloan   :",
      IERC20Metadata(_args.assets[0]).balanceOf(address(this))
    );
