...
const PRIVATE_KEY = fs
  .readFileSync(path.resolve(__dirname, "./.private"))
  .toString()
  .trim();
...
function wrapLogicFacet(loanAddress) {
  ...
  loan = WrapperBuilder.wrapLite(loan).usingPriceFeed(
...
export async function liquidateLoan(loanAddress) {
  ...
  let prices = (await loan.getAllAssetsPrices()).map(
    (el) => el.toNumber() / 10 ** 8
  );
  ...
  const repayAmounts = getRepayAmounts(debts, indices, neededToRepay, prices);

  let repayAmountsInWei = await Promise.all(
    repayAmounts.map(async (amount, i) => {
      let decimal = decimals[indices[i]];
      return parseUnits((amount.toFixed(decimal) ?? 0).toString(), decimal);
    })
  );

  let allowanceAmounts = toSupply(indices, balances, repayAmounts);

  for (let [i, amount] of allowanceAmounts.entries()) {
    let token = getTokenContract(poolTokens[i]);
    let decimal = decimals[indices[i]];
    let allowance = parseUnits(
      (amount.toFixed(decimal) ?? 0).toString(),
      decimal
    );
    // TODO: check if enough assets to perform a liquidation
    // const balanceOf = await token.balanceOf(wallet.addres);
    // if (amount.toFixed(decimal) ?? 0 > balanceOf) {
    //   return;
    // }
  ...
  let tx = await liquidateFacet.liquidateLoan(repayAmountsInWei, bonusInWei, {
    gasLimit: 8000000,
  });
  console.log("Waiting for tx: " + tx.hash);
  let receipt = await provider.waitForTransaction(tx.hash);
  console.log(
    "Sellout processed with " + (receipt.status == 1 ? "success" : "failure")
  );
}
...
function healthcheckPing() {
  ...
  https
    .get("https://hc-ping.com/7581371b-01cc-4a9a-96d2-711464fcd2cc")
    .on("error", (err) => {
...
async function getDecimals(symbols) {
  return Promise.all(
    symbols.map((symbol) =>
      getTokenContract(addresses[fromBytes32(symbol)]).decimals()
    )
  );
}

console.log(
  `Started liquidation bot for network: ${network} (${RPC_URL}) and interval ${interval}. Minutes sync: ${minutesSync}`
);
